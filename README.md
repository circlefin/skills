...
| [`recover-eco-funds`](https://github.com/circlefin/skills/blob/master/plugins/circle/skills/recover-eco-funds/SKILL.md) | Diagnose and recover USDC from a legacy Circle CLI Eco deposit through an evidence-first self-refund and backing-EOA sweep with onchain verification and explicit approvals. |
...
| [`secure-agent-payments`](https://github.com/circlefin/skills/blob/master/plugins/circle/skills/secure-agent-payments/SKILL.md) | Harden an AI agent that spends money over x402 or the Circle agent wallet. Wallet-side limits cap the damage; this covers the agent itself: keys stay in the CLI session, payment details from untrusted content get verified before use, budgets and audit logs live in the agent's own code, and the loop halts on anomalies. Complements `agent-wallet-policy`. |
