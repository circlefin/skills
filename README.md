...
| [`use-cctpx`](https://github.com/circlefin/skills/blob/master/plugins/circle/skills/use-cctpx/SKILL.md) | Bridge non-USDC tokens (cirBTC, wETH on Ethereum ↔ Arc) across chains using CCTPx, Circle's protocol built on CCTP, via Bridge Kit's CCTPx provider. Covers route/token discovery, server-signed fee quoting, FAST/SLOW transfers, and attestation/forward status tracking. |
